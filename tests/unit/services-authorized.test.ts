...
/** A mutator by name. Read-only exports are gated by their own read permission at the
 *  route, not inside the service, so the heuristic targets verbs that WRITE. */
const MUTATOR = /export\s+(?:async\s+)?function\s+((?:create|update|delete|save|set|apply|approve|reject|publish|unpublish|archive|restore|refund|cancel|adjust|transfer|assign|revoke|grant|import|bulk|enqueue|mint|issue|anonymize|impersonate)[A-Z]\w*)/g;

/** Modules that legitimately hold no permission check. Each entry states why. */
const EXEMPT: Array<[RegExp, string]> = [
  [/src\/lib\/auth\//, "authentication runs BEFORE an actor exists — it cannot require a permission of one"],
...
describe("every exported mutator is authorized server-side", () => {
  ...
  it("server actions", () => {
    ...
    for (const file of walk(resolve(ROOT, "src/server/actions"))) {
      const src = readFileSync(file, "utf8");
      if (/requirePermission|requireAll|requireStaffSession|requireCustomerSession/.test(src)) continue;
...
describe("the TOTP privilege line is derived, not hand-listed", () => {
  it("rolesRequiringTotp() matches the matrix", async () => {
    const { rolesRequiringTotp, ROLE_MATRIX, TOTP_REQUIRED_PERMISSIONS } = await import(
      "@/lib/rbac/catalogue"
    );
