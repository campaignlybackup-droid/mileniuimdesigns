...
export const JOB_KINDS: Record<JobKindKey, JobKindMeta> = {
  ...
  export: {
    ...
    staleAfterSeconds: 900,
    description:
      "Staff-requested export. Requires the resource's read permission, plus customer.export for PII.",
  },
  bulk_edit: {
    systemPermitted: false,
    dedupeKey: null,
    ...
    staleAfterSeconds: 900,
    description:
      "Bulk field edit. Re-checks the single-edit permission PER ROW against the queuing user.",
  },
  recalc_apply: {
    systemPermitted: false,
    dedupeKey: "entity",
    ...
    description:
      "Apply an APPROVED price recalculation. Never enqueued by a cron (hard rule 6).",
  },
  email_batch: {
    systemPermitted: false,
    dedupeKey: null,
  ...
  product_metrics_refresh: {
    ...
    staleAfterSeconds: 900,
    description:
      "Refresh product_market_sort.units_90d and the two reporting rollups. One kind, three tables.",
  },
  consistency_check: {
    systemPermitted: true,
    dedupeKey: "kind",
  ...
  send_email: {
    ...
    staleAfterSeconds: 300,
    description:
      "Send one transactional email. Enqueued inside the webhook transaction so a paid order's confirmation is never lost.",
  },
  analytics_dispatch: {
    systemPermitted: true,
    dedupeKey: null,
  ...
  audit_archive: {
    ...
    staleAfterSeconds: 1800,
    description:
      "Weekly off-site audit copy. Its own kind, NOT 'export' — filing a machine-scheduled archive under a human-only kind is the laundering the rule exists to stop.",
  },
